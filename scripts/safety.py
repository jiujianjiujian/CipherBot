...
REJECTED_LOG = os.path.join(LOG_DIR, "rejected_signals.csv")

def log_rejected_signal(signal: dict, reason: str, detail: str = ""):
    """记录被风控/投票/路由拒绝的信号"""
    os.makedirs(LOG_DIR, exist_ok=True)
    row = {
        ...
        "strategy": signal.get("pattern", signal.get("strategy", "?")),
    ...
    file_exists = os.path.exists(REJECTED_LOG)
    with open(REJECTED_LOG, "a", newline="") as f:
